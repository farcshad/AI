...
def print_path(state):
    ...
    while have_not_reached_origin:
        ...
        if (current_state.mother_state == None):
            ...
            length +=1
    print(length)
def path_string(state):
    have_not_reached_origin = True
    current_state = state
    length = 1
    output_string = "===="
    while have_not_reached_origin:
        output_string += str(current_state.status())
...
def solve_prison_problem(player_start_position,computer_start_position,final_position,structure):
    x_final = final_position[0]
    y_final = final_position[1]

    start_state = State(player_start_position[0], player_start_position[1], computer_start_position[0],
                        computer_start_position[1], None)
    ...
    if final_state == None:
        print(f"computer position ({computer_start_position[0]} , {computer_start_position[1]}) is NOT POSSIBLE")
    else:
        output_string = (f"computer position ({computer_start_position[0]} , {computer_start_position[1]})")
        output_string += str(path_string(final_state))
        print(output_string)
...
for i in range(0,8):
    for j in range(0,8):
        computer_start_position = (i,j)
        solve_prison_problem(player_start_position, computer_start_position, final_position, structure)
